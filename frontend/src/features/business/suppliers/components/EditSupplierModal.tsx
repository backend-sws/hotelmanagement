...
export function EditSupplierModal({ isOpen, onClose, supplier, onSuccess }: EditSupplierModalProps) {
  ...
  useEffect(() => {
    if (isOpen && supplier) {
      reset({
        ...
        state_name: supplier.state_name || null,
        bank_name: supplier.bank_name || '',
        account_number: supplier.account_number || '',
        ifsc_code: supplier.ifsc_code || '',
        branch_name: supplier.branch_name || '',
        upi_id: supplier.upi_id || '',
        opening_balance: supplier.opening_balance ? Number(supplier.opening_balance) : 0,
      ...
      title={
        <div className="flex items-center justify-between w-full pr-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="font-bold text-slate-900 dark:text-white">Edit Supplier</span>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-normal">
                {supplier?.name} ({supplier?.custom_id || `ID: ${supplier?.id}`})
              </p>
            </div>
          </div>
